...
import FirstModel from "../commonModel/commonModel";
...
const NewsById = () => {
  ...
  const { data: pageData, isLoading: pageDataLoading } = useFetchData(
    `event?page_url=${page_url}` // Use page_url from useParams
  );
  ...
  const renderModel = (displayMethod) => {
    switch (displayMethod) {
      case 1:
        return <FirstModel dataKey="event" pageData={pageData} dataPage="page"/>;
      case 2:
        return <SecondModel dataKey="event" pageData={pageData} dataPage="page" />;
      case 3:
        return <ThirdModel dataKey="event" pageData={pageData} dataPage="page"/>;
      case 4:
        return <FourthModel dataKey="event" pageData={pageData} dataPage="page"/>;
        case 5:
          return <FifthModel dataKey="event" pageData={pageData} dataPage="page"/>;
          case 6:
            return <SixthModels dataKey="event" pageData={pageData} dataPage="page"/>;
      default:
        return null; // Handle unexpected values
    }
  };
  const metaTitle =
  pageData?.event?.[0]?.page?.[0]?.meta_title || "Default Title"; // Fallback title if not available
const metaDescription =
  pageData?.event?.[0]?.page?.[0]?.meta_description ||
...
let showUploadAndTable = true;
  return (
    <div>
      ...
      <ScrollToTop/>
      <div>
        {pageData?.event?.map((eventItem) =>
          eventItem?.page?.map((pageItem) => (
            <div key={pageItem.id}>
              ...
              <div className="container mx-auto mt-[3rem]">
                <div className="grid lg:grid-cols-12 md:grid-cols-12 grid-cols-1 gap-x-12">
                  <div className="lg:col-span-8 md:col-span-8">
                    ...
                    {showUploadAndTable && (
                    <>
                      {pageData?.event?.some((event) =>
                        event.page.some(
                          (page) =>
                            page.file_section && page.file_section.length > 0
                        )
                      ) && (
                        <UploddFileTwo dataKey="event" pageData={pageData} dataPage="page" />
                      )}
                      {pageData?.event?.some((event) =>
                        event.page.some(
                          (page) =>
                      ...
                      ) && <TableTwo dataKey="event" pageData={pageData} dataPage="page"/>}
                    </>
                  )}
                  </div>
