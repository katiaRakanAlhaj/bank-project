...
const ThirdModel = ({ pageData, dataKey ,dataPage}) => {
  // Check if pageData and its nested properties exist
  if (!pageData || !pageData[dataKey] || pageData[dataKey].length === 0) {
      ...
      {pageData[dataKey].map((item, itemIndex) => (
        <div key={`item-${itemIndex}`}>
          {item?.[dataPage]?.map((page, pageIndex) => {
            const { images = [], descriptions = [], banner_title } = page;
            return (
